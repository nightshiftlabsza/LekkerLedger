...
import { PLANS, type BillingCycle, type PlanId } from "@/src/config/plans";
...
function getPlanSummaryLabel(planId: Exclude<PlanId, "free">, billingCycle: BillingCycle): string {
    return `${PLANS[planId].label} plan - ${getPlanChargeLabel(planId, billingCycle)}`;
}
...
async function openInlinePaystackPayment(input: {
    ...
    popup.resumeTransaction(input.accessCode, {
        ...
        onError: (error: { message?: string } | undefined) => {
            input.onError(error?.message?.trim() || "Secure payment could not be opened. Please try again.");
        },
    });
}
...
export function useInlinePaidPlanCheckout({
    ...
    const openPaystackCheckout = React.useCallback(async (planId: Exclude<PlanId, "free">, email: string) => {
        ...
        try {
            ...
            setLoadingPlanId(null);
            const message = error instanceof Error ? error.message : "Secure payment could not be opened. Please try again.";
            setEmailError(message);
            setDialogOpen(true);
            endAppMetric("paystack_open_latency", {
                plan_id: planId,
    ...
    const dialog = (
        <PaidPlanCheckoutDialog
            ...
            description="We'll use this email for your receipt and account setup. Then you'll continue to secure Paystack payment."
            ...
            <div className="min-w-0 space-y-4">
                <div className="space-y-3">
                    ...
                    <p className="text-sm leading-6 text-[var(--text-muted)]">
                        We&apos;ll use this email for your receipt and account setup. Then you&apos;ll continue to secure Paystack payment.
                    </p>
                </div>

                <form className="min-w-0 space-y-4" onSubmit={handleDialogSubmit}>
                    <div className="min-w-0 space-y-2">
                        <label htmlFor="checkout-email" className="text-sm font-semibold text-[var(--text)]">
                            Email address
                        </label>
                        <Input
                    ...
                    {requestedPlanId ? (
                        <div className="rounded-[20px] border border-[var(--border)] bg-[var(--surface-raised)] px-4 py-3">
                            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[var(--text-muted)]">
                                Selected plan
                            </p>
                            <p className="mt-1 text-sm font-semibold text-[var(--text)]">
                                {getPlanSummaryLabel(requestedPlanId, billingCycle)}
                            </p>
                        </div>
                    ) : null}
                    ...
                    <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                        ...
                        <Button type="submit" className="w-full sm:w-auto sm:min-w-[180px]" disabled={!!loadingPlanId}>
                            ...
                            {loadingPlanId ? "Opening secure payment..." : "Continue to secure payment"}
                        </Button>
                    </div>
                </form>
            </div>
