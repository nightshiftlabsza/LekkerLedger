...
export function TaskList({ title, items, className = "" }: TaskListProps) {
    ...
    return (
        <div className={`space-y-2 ${className}`}>
            ...
            <div className="glass-panel rounded-2xl divide-y divide-[var(--border-subtle)] overflow-hidden">
                {items.map((item) => {
                    const content = (
                        <div className="flex items-center justify-between gap-3 px-4 py-3 transition-colors hover:bg-[var(--bg-subtle)] group">
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                                <StatusChip variant={item.status} />
                                <span className="type-body-bold text-[var(--text-primary)] truncate">{item.label}</span>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                                {item.timeEstimate && (
                                    <span className="type-overline text-[var(--text-muted)]">{item.timeEstimate}</span>
                                )}
                                <ChevronRight className="h-4 w-4 text-[var(--text-muted)] group-hover:text-[var(--amber-500)] transition-colors" />
                            </div>
                        </div>
                    );
