...
export default function UFilingPage() {
    ...
    React.useEffect(() => {
        ...
        if (period && employees.length > 0 && payslips.length > 0) {
            const date = new Date(period.startDate);
            setRows(generateUFilingData(employees, payslips, date.getMonth(), date.getFullYear()));
        } else {
            setRows([]);
        }
    }, [selectedPeriodId, payPeriods, employees, payslips]);

    const handleDownload = () => {
        const period = payPeriods.find(p => p.id === selectedPeriodId);
        if (!settings || !period) return;
        const csv = rowsToCsv(rows, settings);
        downloadCsv(csv, `uFiling_${period.name.replace(/\s+/g, '_')}.csv`);
    };
            ...
            <Card className="glass-panel border-none shadow-sm">
                <CardContent className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <Label className="font-bold text-sm text-[var(--text-primary)]">Select Locked Period</Label>
                            <p className="text-[10px] text-[var(--text-muted)] uppercase font-black">Only locked months are declarative</p>
                        </div>
                    </div>
