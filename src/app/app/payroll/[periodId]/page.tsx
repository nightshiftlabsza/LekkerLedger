...
import { Save, Lock, FileText, AlertTriangle, CheckCircle2, ArrowLeft, Download, Loader2, Palmtree } from "lucide-react";
...
import { getPayPeriod, savePayPeriod, lockPayPeriod as doLockPayPeriod, getEmployees, logAuditEvent, getSettings, getLeaveForEmployee } from "@/lib/storage";
...
export default function PayPeriodWorkspacePage() {
    ...
    return (
        <>
            ...
            {/* Review mode */}
            {showReview && !isLocked && (
                <Card className="glass-panel border-2 border-[var(--color-success)]/30 mb-6">
                    <CardContent className="p-6 space-y-4">
                        ...
                        <p className="type-body text-[var(--text-secondary)]">
                            Check the summary below. Once locked, this pay period becomes an immutable record.
                        </p>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-[var(--bg-subtle)]">
                                        <th className="text-left px-4 py-2 font-bold text-[var(--text-muted)] text-xs uppercase">Employee</th>
                                        <th className="text-right px-4 py-2 font-bold text-[var(--text-muted)] text-xs uppercase">Hours</th>
                                        <th className="text-right px-4 py-2 font-bold text-[var(--text-muted)] text-xs uppercase">Status</th>
                                    </tr>
                                </thead>
                                    ...
                                    {period.entries.map(entry => {
                                        ...
                                        return (
                                            <tr key={entry.employeeId}>
                                                <td className="px-4 py-3 font-medium text-[var(--text-primary)]">{emp?.name ?? "Unknown"}</td>
                                                <td className="px-4 py-3 text-right font-mono text-[var(--text-secondary)]">
                                                    {entry.ordinaryHours + entry.overtimeHours + entry.sundayHours + entry.publicHolidayHours}h
                                                </td>
                        ...
                        <div className="flex gap-3">
                            <Button
                                variant="outline"
                                onClick={() => { setShowReview(false); setPeriod({ ...period, status: "draft" }); }}
                            ...
                            <Button
                                ...
                                className="flex-1 gap-2 bg-[var(--color-success)] text-white font-bold hover:opacity-90"
                            >
