...
export function ContractRow({
    ...
    return (
        <article className="rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] p-5 space-y-5 shadow-sm transition-all hover:shadow-md">
            {/* Header row */}
            <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
                ...
                <p className="text-xs text-[var(--text-muted)] shrink-0 pt-0.5">
                ...
                {STAGES.map((stageLabel, index) => {
                    ...
                    return (
                        <div key={stageLabel} className="space-y-1.5">
                            ...
                            />
                            <p
                                className={`text-[11px] font-medium ${current ? "text-[var(--text)]" : "text-[var(--text-muted)]"
                                    }`}
                            >
                                {stageLabel}
                            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-1">
                <button
                    ...
                    className="rounded-2xl bg-[var(--primary)] text-white px-5 py-2.5 text-sm font-semibold whitespace-nowrap hover:bg-[var(--primary-hover)] transition-colors"
                >
                    {primaryAction.label}
                </button>
                {secondaryAction && (
                    <button
                        ...
                        className="rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] px-5 py-2.5 text-sm font-medium whitespace-nowrap text-[var(--text)] hover:bg-[var(--surface-2)] transition-colors"
                    >
