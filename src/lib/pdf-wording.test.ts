...
import { buildPayslipPdfTemplateData, generatePayslipPdfBytes } from "./pdf";
...
const payslip: PayslipInput = {
  ...
  payPeriodEnd: new Date("2025-11-30"),
  ordinaryHours: 160,
  overtimeHours: 8,
  sundayHours: 0,
...
const forbiddenSnippets = [
  ...
  "Civic Ledger Design",
  "PAY RECORD DETAILS",
  "EMPLOYER CONTRIBUTIONS (Tax Records)",
  "Prepared from payroll information entered by the employer.",
  "v0.1.0",
];
...
describe("PDF wording smoke tests", () => {
  ...
  it("uses saved payslip dates and the cleaned footer copy", async () => {
    const templateData = buildPayslipPdfTemplateData(employee, payslip, "en");

    expect(templateData).toMatchObject({
      headerPeriodLabel: "NOVEMBER 2025",
      payPeriodLabel: "01 Nov 2025 - 30 Nov 2025",
      daysWorkedLine: "Days Worked: 22 days",
      paymentDateLine: "Payment Date: 30 Nov 2025",
      employeeIdLine: "ID / Passport: 8001010234081",
      occupationLine: "Occupation: Domestic Worker",
      leaveSummaryLine: null,
      footerLine: "Payslip generated by LekkerLedger - Household Payroll Simplified",
      netPayLabel: "NET AMOUNT PAID",
    });

    const bytes = await generatePayslipPdfBytes(employee, payslip, settings);
    const text = decodePdf(bytes);
    expect(text).not.toContain("PAY RECORD DETAILS");
    expect(text).not.toContain("EMPLOYER CONTRIBUTIONS (Tax Records)");
    expect(text).not.toContain("Prepared from payroll information entered by the employer.");
    expect(text).not.toContain("MARCH 2026");
  });
