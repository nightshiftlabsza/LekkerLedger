...
export async function generateBCEASummaryPdf(
    ...
    // Official Header
    t("LekkerLedger", PDF_LAYOUT.MARGIN, height - 60, 22, serifBold, PDF_COLORS.TEXT);
    t("BCEA COMPLIANCE SUMMARY", width - PDF_LAYOUT.MARGIN - serifBold.widthOfTextAtSize("BCEA COMPLIANCE SUMMARY", 18), height - 60, 18, serifBold, PDF_COLORS.PRIMARY_GREEN);
    ...
    // Footer
    page.drawLine({ start: { x: PDF_LAYOUT.MARGIN, y: 38 }, end: { x: width - PDF_LAYOUT.MARGIN, y: 38 }, thickness: 0.5, color: PDF_COLORS.BORDER });
    const footerText = "LekkerLedger.app · Civic Ledger Design · BCEA, COIDA & UIF Compliance Engine";
