...
/**
 * Generates a Certificate of Service record (BCEA Section 42)
 */
export async function generateCertificateOfServicePdf(
    ...
    t("LekkerLedger.app · Generated service record", width / 2 - sansRegular.widthOfTextAtSize("LekkerLedger.app · Generated service record", 7) / 2, 25, 7, sansRegular, PDF_COLORS.TEXT_MUTED);

    return pdfDoc.save();
}

/**
 * Generates a payslip checks summary PDF
 */
export async function generateBCEASummaryPdf(
    ...
    // Official Header
    t("LekkerLedger", PDF_LAYOUT.MARGIN, height - 60, 22, serifBold, PDF_COLORS.TEXT);
    t("Payroll Checks Summary", width - PDF_LAYOUT.MARGIN - serifBold.widthOfTextAtSize("Payroll Checks Summary", 18), height - 60, 18, serifBold, PDF_COLORS.PRIMARY_GREEN);
    ...
    t("This sheet summarises the stored wage, deduction, and leave calculations for the selected payslip.", 48, cy, 9, sansRegular, PDF_COLORS.TEXT_MUTED);
    cy -= 30;

    // Table rows
    const rows = [
        ["Check", "Status", "Details"],
        ["National Minimum Wage", breakdown.complianceWarnings.length === 0 ? "Meets check" : "Needs review", `R${employee.hourlyRate.toFixed(2)}/hr`],
        ...
        ["Accommodation Cap", (breakdown.deductions.accommodation ?? 0) <= breakdown.grossPay * 0.1 ? "Within check" : "Above check", "Max 10% allowed"],
        ["Sunday Pay Rate", employee.ordinarilyWorksSundays ? "1.5x" : "2.0x", "Matches the stored Sunday-work setting"],
        ["Overtime Rate", "1.5x", "Based on Sectoral Determination 7 (SD7) reference"],
    ...
    cy -= 30;
    const decl = "This document is a calculation summary generated from the payslip data on record. Review the hours, deductions, and reference rates before sharing or relying on it.";
    page.drawText(decl, { x: 48, y: cy, size: 9, font: sansRegular, color: PDF_COLORS.TEXT, maxWidth: width - 96, lineHeight: 14 });

    // Footer
    page.drawLine({ start: { x: PDF_LAYOUT.MARGIN, y: 38 }, end: { x: width - PDF_LAYOUT.MARGIN, y: 38 }, thickness: 0.5, color: PDF_COLORS.BORDER });
    const footerText = "LekkerLedger.app · Generated by LekkerLedger";
