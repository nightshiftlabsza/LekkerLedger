...
export async function generateRoePayrollPdfBytes(
    ...
    const drawHeader = (page: PDFPage) => {
        ...
        t(page, "LekkerLedger Return of Earnings (ROE) Pack", PDF_MARGIN, pageState.y, { font: serifBold, size: 18, maxWidth: width / 2 });
    ...
    t(pageState.page, "Generated by LekkerLedger for your Compensation Fund records", totalPageWidth - PDF_MARGIN, 40, { size: 7, color: PDF_COLORS.TEXT_MUTED, align: "right" });
...
export async function generateEmployerConfirmationPdfBytes(
    ...
    t("EMPLOYER DETAILS REVIEW", width / 2, currentY, { font: serifBold, size: 16, align: "center", color: PDF_COLORS.PRIMARY_GREEN, maxWidth: width - (PDF_MARGIN * 2) });
    ...
    currentY -= 15;
    t("Review these details before using them in your Return of Earnings (ROE) submission.", PDF_MARGIN, currentY, { size: 9, maxWidth: width - (PDF_MARGIN * 2) });
